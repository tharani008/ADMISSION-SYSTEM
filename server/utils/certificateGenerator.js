...
const generateCertificate = async (application, partnerLogoUrls = [], logoOptions = {}, templateUrl = null) => {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            // coordinates for 1414 x 2000 (Fine-tuned CM nudges down)
            const lineY_Admission = 889;
            const lineY_Cert = 895;      // 892 + 0.1cm
            const lineY_Name = 994;
            const lineY_Course = 1082;    // 1079 + 0.1cm
            const lineY_Duration = 1176;
            const lineY_Issued = 1266;    // 1263 + 0.1cm
            const lineY_Software = 1358;  // 1352 + 0.2cm
            const lineY_Branch = 1447;
            ...
            if (softwareText) {
                ...
                doc.text(parts.join(', '), softwareStartX, lineY_Software, { width: softwareMaxWidth });
            }

            const branchText = application.branch ? `LASAK EDU, ${application.branch}` : 'LASAK EDU';
            doc.fontSize(standardFontSize).font('Helvetica-Bold')
                .text(branchText.toUpperCase(), valX_Main, lineY_Branch);
